defmodule AppGen.InputArguments.Utils do
  ...
  def resolver_function_crud_action(resolver_module_function) do
    case Regex.run(~r/Resolvers\.[[:alpha:]]+\.([^\/]+)/, resolver_module_function,
      ...
      _ ->
        Logger.warn(
          "[AppGen.InputArgumentsReflector] Couldn't find crud action for resolver: #{resolver_module_function}"
        )
  ...
  def update_absinthe_schema_mutations_and_queries(
  ...
  def update_absinthe_schema_type_struct(absinthe_generator_structs, ecto_schema, update_fn) do
    case Enum.find_index(
           absinthe_generator_structs,
           &(is_struct(&1, AbsintheGenerator.Type) and

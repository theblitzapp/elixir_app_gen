defmodule AppGen.AbsintheTypeMerge do
  ...
  def maybe_merge_types(absinthe_generator_structs) do
    {non_type_structs, duplicate_type_structs_map} =
        ...
        fn type_struct, acc_type_struct ->
          %{
            ...
            | enums: merge_enum_objects(acc_type_struct.enums, type_struct.enums),
              objects:
                merge_type_objects(
                  acc_type_struct.type_name,
                  acc_type_struct.objects,
                  type_struct.objects
                )
          }
        end
    ...
    resolved_fields =
      ...
      |> Enum.map(fn {_name, fields} ->
        Enum.reduce(fields, fn field_a, field_b ->
          cond do
            is_nil(field_a.resolver) and not is_nil(field_b.resolver) ->
              field_b

            is_nil(field_b.resolver) and not is_nil(field_a.resolver) ->
              field_a

            field_a.type =~ ~r/list/ and not (field_b.type =~ ~r/list/) ->
              raise IO.ANSI.format([
                      :red,
                      "Found two duplicate types for #{type_name}, one with list and one without",
                      :reset,
                      :orange,
                      "\nA: #{inspect(field_a)}",
                      "\nB: #{inspect(field_b)}"
            ...
            true ->
              raise IO.ANSI.format([
                      :red,
                      "Found two duplicate #{field_a.name} for #{type_name} with different types",
                      :reset,
                      :orange,
  ...
  def remove_relations(absinthe_generator_structs, ecto_struct, relation_key)
      when is_atom(relation_key) do
    remove_relations(absinthe_generator_structs, ecto_struct, [relation_key])
  end

  def remove_relations(absinthe_generator_structs, ecto_struct, relation_key)
      when is_binary(relation_key) do
    remove_relations(absinthe_generator_structs, ecto_struct, [relation_key])
  end
    ...
    Enum.map(absinthe_generator_structs, fn
      %AbsintheGenerator.Type{} = type ->
        %{
          type
          | objects:
              Enum.map(type.objects, fn object ->
                %{
                  object
                  | fields:
                      Enum.reject(object.fields, fn field ->
                        not is_nil(field.resolver) and
                          object.name =~
                            EctoSchemaReflector.ecto_module_resource_name(ecto_struct) and
